...
class Dataset(object):
    def __init__(self, dataset_name,encoders, beta=0, go_backwards=False, pad_mode=PadMode.POST):
        # Public properties
        ...
        for case_index in range(self.num_cases):  #生成图
            ...
            if self.case_lens[case_index]>1:
                ...
                for attr_index in range(self.num_attributes):
                    xs.append(torch.LongTensor(self.features[attr_index][case_index]))
    ...
    def mask(self):
        if self._mask is None:
            self._mask = np.zeros(self._features[0].shape, dtype=bool)
    ...
    @staticmethod
    def _from_event_log(event_log, encoders,include_attributes=None):
        """
        ...
        for i, case in enumerate(event_log.cases):
            ...
            for event in [start_event] + case.events + [end_event]:
                for attribute in event_log.event_attribute_keys: #event_attribute_keys：带顺序排序后的列表
        ...
        # Data preprocessing
        for key, attribute_type in zip(event_log.event_attribute_keys, attr_types):#event_attribute_keys：带顺序排序后的列表
            # Integer encode categorical data
            if attribute_type == AttributeType.CATEGORICAL:
                encoder = encoders[key]
                feature_columns[key] = encoder.transform(feature_columns[key]) + 1
        ...
        for i, (offset, case_len) in enumerate(zip(offsets, case_lens)):
            for k, key in enumerate(event_log.event_attribute_keys):
                x = feature_columns[key]
                features[k][i, :case_len] = x[offset:offset + case_len]

        return features, case_lens, attr_types
    ...
    def from_event_log(self, event_log):
        ...
        self._features, self._case_lens, self.attribute_types= self._from_event_log(event_log,self.encoders)
